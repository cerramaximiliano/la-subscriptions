    ...
    activeSessions: [
      {
        ...
        refreshTokenId: { type: String }, // ID del refresh token como string
...
UserSchema.methods.addSession = function (sessionData) {
  ...
  // Limitar a máximo 5 sesiones activas
  if (this.activeSessions.length > 5) {
    ...
    // Obtener las sesiones que se van a eliminar
    const sessionsToRemove = this.activeSessions.slice(5);
    
    // Revocar los refresh tokens de las sesiones que se eliminarán
    if (sessionsToRemove.length > 0) {
      const RefreshToken = require('./RefreshToken');
      const refreshTokenIds = sessionsToRemove
        .filter(session => session.refreshTokenId)
        .map(session => session.refreshTokenId);
      
      if (refreshTokenIds.length > 0) {
        // Marcar como revocados los refresh tokens de las sesiones eliminadas
        RefreshToken.updateMany(
          { _id: { $in: refreshTokenIds } },
          { isRevoked: true }
        ).catch(err => {
          const logger = require('../utils/logger');
          logger.error('Error al revocar refresh tokens de sesiones antiguas:', err);
        });
      }
    }
    
    // Mantener solo las 5 más recientes
    this.activeSessions = this.activeSessions.slice(0, 5);
  }
...
UserSchema.methods.removeSession = function (deviceId) {
  ...
  const sessionToRemove = this.activeSessions.find(s => s.deviceId === deviceId);
  
  // Si existe y tiene refreshTokenId, revocarlo
  if (sessionToRemove && sessionToRemove.refreshTokenId) {
    const RefreshToken = require('./RefreshToken');
    RefreshToken.updateOne(
      { _id: sessionToRemove.refreshTokenId },
      { isRevoked: true }
    ).catch(err => {
      const logger = require('../utils/logger');
      logger.error('Error al revocar refresh token de sesión eliminada:', err);
    });
  }
...
UserSchema.methods.removeAllSessionsExcept = function (deviceId) {
  // Obtener las sesiones que se van a eliminar
  const sessionsToRemove = this.activeSessions.filter(s => s.deviceId !== deviceId);
  ...
  if (sessionsToRemove.length > 0) {
    ...
    if (refreshTokenIds.length > 0) {
      RefreshToken.updateMany(
        ...
        const logger = require('../utils/logger');
        logger.error('Error al revocar refresh tokens de sesiones eliminadas:', err);
      });
    }
  }
